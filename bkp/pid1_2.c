...
// Frames por segundo da simulação
#define SIM_FPS 60
...
// Limites de ganhos do PID para evitar instabilidades
static const float Kp_min = 0.0f;
static const float Kp_max = 1000.0f;
static const float Ki_min = 0.0f;
static const float Ki_max = 1.0f;
static const float Kd_min = 0.0f;
static const float Kd_max = 1000.0f;
...
// Amplitude de ruído no ângulo medido
static float angleNoiseAmplitude = 0.005f;

// Saturação física do torque do motor (N·m) - Exemplo: limite arbitrário
static const float MAX_MOTOR_TORQUE = 50.0f;
...
static bool useRK4 = false;

//------------------------------------------------------------------------------
// Perturbação Externa Parametrizável
//------------------------------------------------------------------------------
static bool externalDisturbanceEnabled = false;
static float externalDisturbanceMagnitude = 0.0f;
// A ideia é que, quando habilitada, a perturbação seja +/- a variável definida

//------------------------------------------------------------------------------
// Funções Auxiliares
//------------------------------------------------------------------------------

// Função PID - cálculo da saída de controle
...
// Mapeamento de valor de uma faixa para outra
...
// Desenha uma bússola (apenas visual)
static void DrawCompass(float centerX, float centerY, float radius)
{
...
// Adiciona ruído à leitura de ângulo
...
static void ResetSimulation(void)
{
    ...
    externalDisturbanceMagnitude = 0.0f;
    externalDisturbanceEnabled = false;
...
// Função para calcular o torque total (gravidade, atrito, motor e externo)
...
// Programa Principal
//------------------------------------------------------------------------------
// Observação sobre Validação Experimental:
// É recomendado que os parâmetros (massa, atrito, deslocamento, etc.) sejam
// validados experimentalmente para garantir que a simulação corresponda ao
// sistema real.
//
// Sobre Modularidade:
// Em uma aplicação real, dividiríamos este código em múltiplos arquivos
// (.c/.h), por exemplo: control.c, physics.c, graphics.c, mantendo o main.c
// mais limpo. Aqui está tudo em um único arquivo para fins de demonstração.
int main(void)
{
    ...
    InitWindow(1240, 768, "Simulacao PID - Motor Hidráulico");
    ...
    while (!WindowShouldClose())
    {
        ...
        // Ajuste manual da pressão com teclas UP/DOWN
        ...
        // Ajuste dinâmico de Kp, Ki, Kd com saturação
        ...
        // Saturando os ganhos para evitar valores que possam desestabilizar o
        // sistema
        if (Kp < Kp_min)
            Kp = Kp_min;
        if (Kp > Kp_max)
        ...
        // Alternar método de integração (TAB)
        if (IsKeyPressed(KEY_TAB))
            useRK4 = !useRK4;

        // Perturbação externa: Tecla P habilita/desabilita
        if (IsKeyPressed(KEY_P))
        {
            externalDisturbanceEnabled = !externalDisturbanceEnabled;
        }

        // Ajuste da magnitude da perturbação externa (J/K)
        if (IsKeyPressed(KEY_J))
            externalDisturbanceMagnitude += 0.5f;
        if (IsKeyPressed(KEY_K))
            externalDisturbanceMagnitude -= 0.5f;
        if (externalDisturbanceMagnitude < 0.0f)
            externalDisturbanceMagnitude = 0.0f;

        // Cálculo da perturbação externa (se habilitada)
        float externalTorque = 0.0f;
        if (externalDisturbanceEnabled)
        {
            externalTorque = ((float)rand() / (float)RAND_MAX - 0.5f) * 2.0f *
                             externalDisturbanceMagnitude;
        }

        // Leitura do ângulo medido (com ruído)
        ...
        // Duty Cycle (apenas para visualização)
        float dutyCycle =
        ...
        // Aplicando saturação no torque do motor
        if (motorTorque > MAX_MOTOR_TORQUE)
            motorTorque = MAX_MOTOR_TORQUE;
        if (motorTorque < -MAX_MOTOR_TORQUE)
            motorTorque = -MAX_MOTOR_TORQUE;
        ...
        //------------------------------------------------------------------------------
        // Desenho na tela (Gráficos e Informações)
        //------------------------------------------------------------------------------
        BeginDrawing();
        ...
        DrawText("Pressione R para RESETAR a simulacao",
                 10, 670, 20, (Color){255, 200, 100, 255});
        DrawText("P para habilitar/desabilitar perturbação, J/K para ajustar a "
